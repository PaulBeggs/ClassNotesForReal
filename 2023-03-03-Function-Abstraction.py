...
#        -- not to main() or to h1(), unless h1() goes back to g1(), which
...
# Code Breaker
#
# We are going to write a simple human vs computer
...
def set_level() -> int:
    ...
    medium = 4
    hard = 5
# Easy, Medium, and Hard are ONLY defined in this function.
...
def human_guess(code): # we are only looking at code here so that they match in length!
    ...
    is_okay = False
    while not is_okay:
        guess = input(f'Please enter a guess of length {len(code)}. ').upper()
